...
def test_connection_collection():
    ...
    subtest_can_access_src_connections_from_mixer(session, mixer, [connection1])
...
def test_creating_connection_from_input_to_mixer():
    ...
    mixer = session.mixers.add()
    input = session.inputs.add(type='test_video')
    assert mixer.connection_for_src(input) == None
    connection1 = mixer.connection_for_src(input, create_if_not_made=True)
    assert connection1.src == input
    assert connection1.dest == mixer
    connection1_copy = mixer.connection_for_src(input, create_if_not_made=True)
...
def test_creating_connection_from_mixer_to_output():
    ...
    connection2 = output.connection_for_src(mixer, create_if_not_made=True)
    assert connection2.src == mixer
    assert connection2.dest == output
    connection2_copy = output.connection_for_src(mixer, create_if_not_made=True)
...
def subtest_can_find_src_and_dest(cc, input, mixer, output1, connection1, connection2, connection3):
    assert cc.get_first_collection_for_src(input) == connection1
    assert cc.get_first_collection_for_src(mixer) == connection2
    assert cc.get_first_collection_for_src(output1) == None

    assert cc.get_all_collections_for_src(input) == [connection1, connection3]
    assert cc.get_all_collections_for_src(mixer) == [connection2]
    assert cc.get_all_collections_for_src(output1) == []

    assert cc.get_first_collection_for_dest(input) == None
    assert cc.get_first_collection_for_dest(mixer) == connection1
    assert cc.get_first_collection_for_dest(output1) == connection2

    assert cc.get_all_collections_for_dest(input) == []
    assert cc.get_all_collections_for_dest(mixer) == [connection1]
    assert cc.get_all_collections_for_dest(output1) == [connection2]
...
def subtest_can_access_src_connections_from_mixer(session, mixer, connections):
    assert mixer.src_connections() == connections
...
def subtest_can_access_connections_from_output(session, output, connection):
    assert output.src_connection() == connection
